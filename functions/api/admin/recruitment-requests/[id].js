...
export async function onRequestGet({ env, params }) {
  return safeHandler(async () => {
    const row = await first(env, 'SELECT * FROM recruitment_requests WHERE id = ?', [params.id]);
...
export async function onRequestPatch({ request, env, params }) {
  return safeHandler(async () => {
    const existing = await first(env, 'SELECT * FROM recruitment_requests WHERE id = ?', [params.id]);
