...
function renderRequests() {
  const body=$('requestTableBody'); body.innerHTML=requests.length?requests.map(r=>`<tr class="border-b last:border-0"><td class="px-5 py-3.5 font-medium">${RBP.escapeHtml(r.company_name)}</td><td class="px-5 py-3.5 text-xs"><p>${RBP.escapeHtml(r.contact_name)}</p><p>${RBP.escapeHtml(r.email)}</p></td><td class="px-5 py-3.5">${RBP.escapeHtml(r.position_title)}</td><td class="px-5 py-3.5 text-xs">${RBP.formatDate(r.created_at)}</td><td class="px-5 py-3.5"><select class="request-status border rounded-lg px-2 py-1 text-xs" data-id="${r.id}">${REQUEST_STATUSES.map(s=>`<option ${r.status===s?'selected':''}>${s}</option>`).join('')}</select></td><td class="px-5 py-3.5 text-right"><button class="view-request text-teal font-semibold text-sm" data-id="${r.id}">View</button></td></tr>`).join(''):'<tr><td colspan="6" class="text-center py-10 text-navy/40">No recruitment requests have been received yet.</td></tr>';
...
function renderInterest() {
  const body=$('interestTableBody'); body.innerHTML=interest.length?interest.map(c=>`<tr class="border-b last:border-0"><td class="px-5 py-3.5 font-medium">${RBP.escapeHtml(c.name)}</td><td class="px-5 py-3.5 text-xs"><p>${RBP.escapeHtml(c.email)}</p><p>${RBP.escapeHtml(c.phone||'')}</p></td><td class="px-5 py-3.5 text-sm">${RBP.escapeHtml([c.preferred_roles,c.preferred_location].filter(Boolean).join(' · ')||c.message||'—')}</td><td class="px-5 py-3.5 text-xs">${RBP.formatDate(c.created_at)}</td><td class="px-5 py-3.5"><select class="interest-status border rounded-lg px-2 py-1 text-xs" data-id="${c.id}">${INTEREST_STATUSES.map(s=>`<option ${c.status===s?'selected':''}>${s}</option>`).join('')}</select></td></tr>`).join(''):'<tr><td colspan="5" class="text-center py-10 text-navy/40">No candidate interest registrations yet.</td></tr>';
