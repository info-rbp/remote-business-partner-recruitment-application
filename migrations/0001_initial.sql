-- Remote Business Partner — D1 schema (initial migration)
-- Safe to apply with Wrangler even if the runtime bootstrap has already
-- initialised the database, because all objects are created IF NOT EXISTS.
...
CREATE TABLE IF NOT EXISTS vacancies (
    ...
    job_type TEXT NOT NULL
        CHECK (job_type IN ('Full-Time','Part-Time','Casual','Contract','Temporary')),
...
CREATE TABLE IF NOT EXISTS recruitment_requests (
    ...
    status TEXT NOT NULL DEFAULT 'New'
        CHECK (status IN ('New','Contacted','Engaged','Closed')),
    privacy_acknowledged INTEGER NOT NULL
        CHECK (privacy_acknowledged IN (0,1)),
    privacy_acknowledged_at TEXT NOT NULL,
    created_at TEXT NOT NULL,
